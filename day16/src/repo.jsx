...
class PopularRepoList extends React.Component {
  ...
  async componentDidMount() {
    try {
      ...
      const res = await axios.get(
        "https://api.github.com/search/repositories?q=stars>1&sort=stars&order=desc&type=Repositories"
      );
  ...
  render() {
    ...
    const list = repos.map((item) => (
      <ListGroup.Item key={item.id}>{item.full_name}</ListGroup.Item>
    ));
    return (
      <div>
        {loading && <Spinner animation="border" variant="success" />}
        {errorText && <Alert variant="danger">{errorText}</Alert>}
        {repos.length > 0 && <ListGroup>{list}</ListGroup>}
      </div>
    );
  }
}
